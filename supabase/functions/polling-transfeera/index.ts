...
Deno.serve(async (req) => {
    ...
    try {
        ...
        if (!id_pix) {
            throw new Error("Parâmetro 'id_pix' é obrigatório.")
        }
        ...
        // Endpoint: GET /pix/qrcode/{id} (Descoberto pelo usuário)
        ...
        if (!responseCtx.ok) {
            ...
            if (responseCtx.status === 404) {
                return new Response(JSON.stringify({ status: 'NOT_FOUND', message: 'QRCode Pix não encontrado na Transfeera' }), {
        ...
        // No JSON enviado pelo usuário, o status vem na raiz como "CONCLUIDA"
        ...
        if (successStatuses.includes(status)) {
            ...
            try {
                // Busca dados do usuário (Saldo autal e Nome)
                ...
                if (userError || !usuarioData) {
                    throw new Error(`Usuário não encontrado para atualizar saldo: ${userError?.message}`);
                }
                ...
                console.log(`Saldo atualizado para usuário ${userId}. Novo saldo: ${novoSaldo}`);
                ...
                await supabaseClient.from('transfeera_log').insert({
                    ...
                    response_payload: { message: 'O QRCode foi pago com sucesso na Transfeera.' },
                    http_status: 200
                })
                ...
                await supabaseClient.from('transfeera_log').insert({
                    ...
                    response_payload: { message: `Foi inserido o valor na conta do aluno ${nomeAluno}.` },
                    http_status: 200
                })

            } catch (processError: any) {
                console.error("Erro no processamento pós-confirmação (Saldo/Logs):", processError);
                // Não lançamos erro fatal aqui para não reverter o status 'PAGO' do investimento, caso contrário o loop do front continuaria tentando pagar
            }
        ...
        return new Response(JSON.stringify({
            ...
            message: 'Ainda não identificado como pago.',
            data: pixData
        }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 200,
